...
char in_path[MAX_STRING], suffix[MAX_STRING], out_file[MAX_STRING], featureid_file[MAX_STRING], fieldid_file[MAX_STRING], cluster_file[MAX_STRING];
int debug_mode = 2;
...
map<string, map<pair<int,int>, int> > g_mapResFeatures;//map<uid, map<<fieldid, featureid>, value>>
...
int main(int argc, char **argv) {
    ...
    if (argc == 1) {
        ...
        printf("\nExamples:\n");
        printf("./predict_ffm_f -in ./ -suffix txt -clus conf/cluster.conf -fid out/ffm_features.txt.fieldid -featid out/ffm_features.txt.featureid -out out/ffm_features_pre.txt -debug 2\n\n");
        return 0;
    }
    in_path[0] = 0;
    out_file[0] = 0;
    ...
    if(debug_mode >= 1){
        cout << "parameters:"<< endl
            << "-in:" << in_path << endl
            << "-suffix:" << suffix << endl
            << "-fieldid:" << fieldid_file << endl
            << "-featureid:" << featureid_file << endl
