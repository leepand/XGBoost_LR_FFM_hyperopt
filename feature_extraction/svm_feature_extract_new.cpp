...
int GetFid(string filename, string featurename, string strvalue, FEATURE_TYPE_E type){
    ...
    if(fname.length() > 0){
        ...
        if(it == g_mapFids.end()){
            pthread_mutex_lock(pmutex);
            g_mapFids[fname]=g_fid++;//g_mapFids.size();
...
bool GetMergeFIDs(int &mfeatureid,
    ...
    if(hasMF && mkindex > -1 && mvindex > -1){
        ...
        if(itfeat != g_mapFids.end()){
            ...
            pthread_mutex_lock(pmutex);
            g_mapFids[featname] = g_fid++;//g_mapFids.size();
...
int main(int argc, char **argv) {
    ...
    if (argc == 1) {
        ...
        printf("\nExamples:\n");
        printf("./svm_feature_extract -in ./ -suffix txt -feid 1000 -clus conf/cluster.conf -out out/svm_features.txt -debug 2\n\n");
        return 0;
    }
    in_path[0] = 0;
    out_file[0] = 0;
    ...
    if ((i = ArgPos((char *)"-feid", argc, argv)) > 0) g_fid = atoi(argv[i + 1]);
