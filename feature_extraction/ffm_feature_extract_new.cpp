...
char in_path[MAX_STRING], suffix[MAX_STRING], out_file[MAX_STRING], cluster_file[MAX_STRING];
int debug_mode = 2;
...
map<string, map<pair<int,int>, int> > g_mapResFeatures;//map<user, map<<fieldid, featureid>, value>>
...
static int g_fiid = 0;//new field id
static int g_feid = 0;//new feature id
...
/*int GetUid(string cookie){
    ...
    if(it != g_mapUids.end()){
        ...
        g_mapUids[cookie] = ++g_uid;//g_mapUids.size();
...
int GetFieldID(string filename, string featurename, FEATURE_TYPE_E type){
    ...
    if(fname.length() > 0){
        ...
        if(it == g_mapFields.end()){
            pthread_mutex_lock(pmutex);
            g_mapFields[fname]=g_fiid++;//g_mapFields.size();
...
int GetFeatureid(string filename, string featurename, string strvalue, FEATURE_TYPE_E type){
    ...
    if(fname.length() > 0){
        ...
        if(it == g_mapFeatures.end()){
            pthread_mutex_lock(pmutex);
            g_mapFeatures[fname]=g_feid++;//g_mapFeatures.size();
...
bool GetMergeFIDs(int &mfieldid,
    ...
    if(hasMF && mkindex > -1 && mvindex > -1){
        ...
        if(itfld != g_mapFields.end()){
            ...
            pthread_mutex_lock(pmutex);
            g_mapFields[fldname] = g_fiid++;//g_mapFields.size();
        ...
        if(itfeat != g_mapFeatures.end()){
            ...
            pthread_mutex_lock(pmutex);
            g_mapFeatures[featname] = g_feid++;//g_mapFeatures.size();
...
bool AddFeature(int fieldid, int featureid, string uid, int val, bool isCVal = false){
    if(fieldid < 0 || featureid < 0 || uid.length() <= 0 || val < 0) return false;
...
int main(int argc, char **argv) {
    ...
    if (argc == 1) {
        ...
        printf("\t-feid <int>(optional)\n");
        printf("\t\t The base id value for new added feature, default 0\n");
        ...
        printf("\nExamples:\n");
        printf("./ffm_feature_extract -in ./ -suffix txt -feid 1000 -clus conf/cluster.conf -out out/ffm_features.txt -debug 2\n\n");
        return 0;
    }
    in_path[0] = 0;
    out_file[0] = 0;
    ...
    if ((i = ArgPos((char *)"-feid", argc, argv)) > 0) g_feid = atoi(argv[i + 1]);
